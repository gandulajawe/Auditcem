import { db } from "@/db";
import { auditChecklists, weeklyCadence, auditReports } from "@/db/schema";
import { count } from "drizzle-orm";

export async function ensureInitialData() {
  try {
    // Check if checklists exist
    const [{ value: checklistCount }] = await db.select({ value: count() }).from(auditChecklists);
    
    if (Number(checklistCount) === 0) {
      console.log("Seeding initial checklist data...");
    ...
    // Check if weekly cadence exists
    const [{ value: cadenceCount }] = await db.select({ value: count() }).from(weeklyCadence);

    if (Number(cadenceCount) === 0) {
      console.log("Seeding initial weekly cadence data...");
    ...
    const [{ value: reportCount }] = await db.select({ value: count() }).from(auditReports);

    if (Number(reportCount) === 0) {
      console.log("Seeding initial audit reports data...");
